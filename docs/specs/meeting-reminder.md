...
The meeting section lists today's meetings that have not yet ended, split into two visually separated groups: meetings **in progress** ("Now") and **upcoming** meetings ("Up next"). The section scrolls when there are many meetings. A meeting stays listed until its end time has passed, then it drops out. Each meeting shows its title and its start and end times in the user's locale format.
