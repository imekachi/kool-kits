...
The header strip carries identity and connection status: the Kool Kits mark and name, and the connected account's email beside a status indicator (a green dot when a live session is identified, a muted dot reading "Not connected" otherwise). Surfacing the account in the popup — not only on the options page — answers "whose calendar am I looking at?" at the moment the user is reading the list, which matters because the badge and list are only trustworthy if the user can confirm the source. The header is hidden entirely when the feature is disabled, since there is then no calendar access to describe.
...
The section is presented as a single framed card under a Kool Kits header (the brand mark and name): a header band pairing the feature name with a one-line description and the master switch, the calendar controls below (each with a short hint explaining what it does), and a connection footer carrying the status indicator. When the feature is off, the calendar controls (lead time and which-meetings) are dimmed and disabled because they no longer apply, while the connection status stays live — connection is independent of the enabled flag, so a connected-but-disabled user still sees their account rather than being prompted to reconnect. The connection footer mirrors the popup header's indicator (a green dot with the account email when connected, a muted "Not connected" with the open-Google-Calendar action otherwise), so the two surfaces speak the same connection language.
