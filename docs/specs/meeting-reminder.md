...
Connecting is therefore not a credential prompt but an instruction: when the status calls for the connect screen (`DISCONNECTED` or `SESSION_UNAVAILABLE`), the extension offers to open Google Calendar in a new tab. A successful connect path promotes the status to `CONNECTED` and loads meetings. The extension holds no OAuth grant to revoke. Ending Google access in the browser moves the status to `SESSION_UNAVAILABLE` on a later sync. Separately, when the extension shows `CONNECTED`, the user can **disconnect** in options: that sets `DISCONNECTED`, wipes cached meetings and the connected email, clears the badge, and stops calendar syncing until the user connects again. Disconnect does not sign the user out of Google.
...
The displayed countdowns advance while the popup stays open, recomputed from the cached meetings on a short interval so "ends in 8m" and "in 4m" do not go stale during a long glance, without requiring another fetch.
